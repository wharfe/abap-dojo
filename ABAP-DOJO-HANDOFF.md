...
- **実装上の訂正（2026-09-02 実測）**: トランスパイラの構文バージョンは 7.02 ではなく
  **`open-abap`**（`@abaplint/transpiler` の `config.syntax.version`）。したがって
  ...
  詰まりの一部は意味解析 — `STRING_TABLE`・DDIC テーブル・`CL_*` クラスが
  open-abap-core に無い、という欠落が `syntax_error` に落ちている。
  ただし **「主成分が標準アーティファクトの欠落」とはまだ言えない**: abaplint は
  型不整合なども同じ `check_syntax` キーで発行するため、欠落と純粋なコードの誤りが1つの
  バケツに混ざる。内訳の判定には別の signal が必要（#56）
- **「詰まるのは構文ではなく意味解析」は言い過ぎだった（2026-09-04 以降の実測）**:
  `syntax_error` の中で最大のバケツは `check_syntax` ではなく **`parser_error`**（構文側）で、
  さらにその中身の相当部分は**他言語由来の記法** — 特に `"` を文字列引用符と誤解した形
  （ABAP では `"` はコメント開始）。#69 でこれに説明を出すようにし、同じ `WRITE` バケツにいた
  ピリオド抜け・行末セミコロンにも #83 で説明を出す（こちらは誤ヒントを避けるため判定が厳しい）
  **数値をここに書かない。** 正本は GA4 と CLAUDE.md の Analytics 節（比率は月単位で動く）
- **その一部は「失敗として現れない」**（#71 で手当て済み）: `WRITE: "hello".` のように
  チェーンの operand をコメントが食うと、残った空チェーンが**正しい ABAP として通る**。
  abaplint は何も言わず run は `success`、出力だけが無い。エラーも警告も出力も無い唯一の
  失敗なので、#69 の「エラーが減るか」で探す仕組みでは原理的に届かない。実文の数が増えるかで
  探す別の入口を足し、`silent_loss` として計測するようにした。
  **ただし直ったのはトップレベル直書きのチェーンだけ** — `IF` や `METHOD` の中の同じ形は
  `structure` エラーになるので、もともと #69 が拾っていた
